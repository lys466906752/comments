...
<script>
	...
	function reads()
	{
		$.ajax({url:"<?php echo masters_url();?>sites/index_ajax/" + pageindex + "?keywords=" + keywords, 
			type: 'GET', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	{
		location="<?php echo masters_url();?>sites/edit/" + id + "/masters.html?pageindex=" + pageindex + "&keywords=" + keywords;	
	}
	...
	function ups(id)
	{
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo admin_url();?>faces/ups/" + id, 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
	...
	{
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo admin_url();?>faces/downs/" + id, 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
	...
	function del_it(id){
		layer.confirm('您确定要删除所选数据吗！删除数据不可恢复！', {
			btn: ['确定','取消'] //按钮
		}, function(){
			del_rs(id);
		}, function(){
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo masters_url();?>sites/del", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 站点管理 <span class="c-gray en">&gt;</span> 站点信息 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c"> 
    <input type="text" class="input-text" style="width:250px" placeholder="站点名称/网址/appKey" id="keys" name="keys" value="<?php echo $keywords;?>">&nbsp;&nbsp;
		<button type="submit" class="btn btn-success radius" id="" name="" onClick="soso();"><i class="Hui-iconfont">&#xe665;</i> 搜内容</button>  &nbsp;&nbsp;&nbsp; <a href="<?php echo masters_url();?>sites/downloads" target="_blank">点击下载映射资源到网站根目录</a>
	</div>	
    <span id="inners">
