...
<script>
	...
	function reads()
	{
		$.ajax({url:"<?php echo masters_url();?>comments/index_ajax/<?php echo $site_id;?>/" + pageindex + "?keywords=" + keywords, 
			type: 'GET', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	{
		location="<?php echo masters_url();?>comments/edit/" + id + "/<?php echo $site_id;?>?pageindex=" + pageindex + "&keywords=" + keywords;	
	}
	...
	function clear_rs()
	{
		...
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo masters_url();?>comments/truncates/" + id, 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo masters_url();?>comments/del", 
			type: 'POST', 
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 评论管理 <span class="c-gray en">&gt;</span> 评论信息 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c"> 
    <input type="text" class="input-text" style="width:250px" placeholder="用户昵称/回复内容关键词" id="keys" name="keys" value="<?php echo $keywords;?>">&nbsp;&nbsp;
