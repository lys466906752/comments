...
<script>
	...
	function reads()
	{
		$.ajax({url:"<?php echo masters_url();?>comments/res_ajax/<?php echo $site_id;?>/" + pageindex + "?keywords=" + keywords + '&reId=' + reId, 
			type: 'GET', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	{
		location="<?php echo masters_url();?>comments/re_edit/" + id + "/<?php echo $site_id;?>?pageindex=" + pageindex + "&keywords=" + keywords + '&reId=' + reId;	
	}
	...
	function clearTable()
	{
		layer.confirm('您确定要清空所有数据吗！清空后数据不可恢复！', {
			btn: ['确定','取消'] //按钮
		}, function(){
			clear_rs();
		}, function(){
	...
	function clear_rs()
	{
		var id="<?php echo $site_id;?>";
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo masters_url();?>comments/res_truncates/" + id, 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo masters_url();?>comments/rs_del", 
			type: 'POST', 
			data:{id:id,siteid:<?php echo $site_id;?>},
			dataType: 'html', 
			timeout: 15000, 
				error: function(){
					layer.closeAll();
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 回复管理 <span class="c-gray en">&gt;</span> 回复信息 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c"> 
    <input type="text" class="input-text" style="width:250px" placeholder="回复人昵称/回复内容关键词" id="keys" name="keys" value="<?php echo $keywords;?>">&nbsp;&nbsp;
