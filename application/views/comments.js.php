			...
			$.ajax({  
				...
				success:function(data){
					if(data.code==100)
					{
						...
						{
							var htmlInner=htmlInner + '<div class="sx_plbt"><img src="<?php echo base_url();?>' + arrays[i]["avatar"] + '" alt="" class="sx_pltx" onclick="javascript:show_members_infos(\'' + arrays[i].uid + '\');"/><span class="sx_mz">' + arrays[i]["nickname"] + '</span><p class="sx_plri"><i>' + arrays[i]["time"] + '</i><span>' + createStar(arrays[i]["star"],arrays[i]["id"]) + '</span></p></div><div class="sx_plcont"><p>' + arrays[i]["fulltext"] + '<a>' + comments_img_show(arrays[i].files,arrays[i].id) + '</a></p><div class="sx_click"><img src="<?php echo base_url();?>assets/images/sx_zan.png" class="sx_zan" onclick="addPraise(\'' + arrays[i].id + '\')" /><span id="praiseInner_' + arrays[i].id + '">' + arrays[i].ok + '</span><img src="<?php echo base_url();?>assets/images/sxpl.png" alt="" class="sx_pl" onclick="showOrHideRe(\'' + arrays[i].id + '\')" /><div class="sx_plreply" id="sx_reply_' + arrays[i]["id"] + '"><textarea  placeholder="就医体验如何？把感受告诉更多人" class="sx_reply1" id="replyItem_' + arrays[i].id + '"></textarea>' + createFaceHtml(arrays[i].id) + '<p><a href="javascript:showFaceDiv(\'' + arrays[i]["id"] + '\')"><img src="<?php echo base_url();?>assets/images/face.png" alt="" /></a><input type="submit" value="发表" class="sx_reply2" onclick="subReplyItem(\'' + arrays[i].id + '\')"/></p></div></div></div><div class="sx_plhf">' + createReItem(arrays[i]["retext"]) + '</div>';
				...
				$.ajax({  
					...
					success:function(data){
						...
						if(data.code==100)
						{
							...
							for(var i=0;i<arrays.length;i++)
							{
								var htmlInner=htmlInner + '<li id="listsview_' + arrays[i].id + '"><div class="sx_plbt"><img src="<?php echo base_url();?>' + arrays[i]["avatar"] + '" alt="" class="sx_pltx" onclick="javascript:show_members_infos(\'' + arrays[i].uid + '\');"/><span class="sx_mz">' + arrays[i]["nickname"] + '</span><p class="sx_plri"><i>' + arrays[i]["time"] + '</i><span>' + createStar(arrays[i]["star"],arrays[i]["id"]) + '</span></p></div><div class="sx_plcont"><p>' + arrays[i]["fulltext"] + '<a>' + comments_img_show(arrays[i].files,arrays[i].id) + '</a></p><div class="sx_click"><img src="<?php echo base_url();?>assets/images/sx_zan.png" class="sx_zan" onclick="addPraise(\'' + arrays[i].id + '\')" /><span id="praiseInner_' + arrays[i].id + '">' + arrays[i].ok + '</span><img src="<?php echo base_url();?>assets/images/sxpl.png" alt="" class="sx_pl" onclick="showOrHideRe(\'' + arrays[i].id + '\')" /><div class="sx_plreply" id="sx_reply_' + arrays[i]["id"] + '"><textarea  placeholder="就医体验如何？把感受告诉更多人" class="sx_reply1" id="replyItem_' + arrays[i].id + '"></textarea>' + createFaceHtml(arrays[i].id) + '<p><a href="javascript:showFaceDiv(\'' + arrays[i]["id"] + '\')"><img src="<?php echo base_url();?>assets/images/face.png" alt="" /></a><input type="submit" value="发表" class="sx_reply2" onclick="subReplyItem(\'' + arrays[i].id + '\')"/></p></div></div></div><div class="sx_plhf">' + createReItem(arrays[i]["retext"]) + '</div></li>';
