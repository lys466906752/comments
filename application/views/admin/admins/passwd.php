...
<article class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<tbody>
           <tr class="text-c" >
			  <td width="177">管理员密码：</td>
			  <td width="826" align="left" valign="middle">
              <input type="password" class="input-text" value="" placeholder="6-16位之间" id="passwd" name="passwd">
              </td>
		  </tr>
			...
			<tr class="text-c" >
              ...
              <button onClick="article_save_submit();" class="btn btn-primary radius" type="submit"><i class="Hui-iconfont">&#xe632;</i> 密 码 重 置   </button>
              </td>
		  </tr>
...
<script>
	...
	function article_save_submit()
	{
		...
		if(form_loads==1)
		{
			...
			{
				layer.msg('管理员密码长度为6-18位之间');	
			}
			else
			{
				$.ajax({url:"<?php echo admin_url();?>admins/passwd_update", 
				type: 'POST', 
				data:{passwd:passwd}, 
				dataType: 'html', 
				timeout: 15000, 
					...
					success:function(result){
						...
						if(result.indexOf("|")>0){
							...
							if(arr[0]==100){
								...
								setTimeout("location='<?php echo admin_url();?>admins/passwd'",1500);
							}else if(arr[0]==200){
