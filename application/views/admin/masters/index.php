...
<script>
	...
	function reads()
	{
		$.ajax({url:"<?php echo admin_url();?>masters/index_ajax/" + pageindex + "?keywords=" + keywords, 
			type: 'GET', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	{
		location="<?php echo admin_url();?>members/edit/" + id + "/masters.html?pageindex=" + pageindex + "&keywords=" + keywords;	
	}
	...
	function del_it(id){
		layer.confirm('删除所选管理员，他们管理的站点也会被删除，不可恢复！', {
			btn: ['确定','取消'] //按钮
		}, function(){
			del_rs(id);
		}, function(){
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo admin_url();?>masters/del", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
...
<div class="page-container">
	<div class="text-c"> 
    <input type="text" class="input-text" style="width:250px" placeholder="管理员手机号/昵称" id="keys" name="keys" value="<?php echo $keywords;?>">&nbsp;&nbsp;
