...
<script>
	...
	function reads()
	{
		$.ajax({url:"<?php echo admin_url();?>masters/sites_ajax/" + pageindex + "?keywords=" + keywords, 
			type: 'GET', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	{
		location="<?php echo admin_url();?>masters/sites_edit/" + id + "/masters.html?pageindex=" + pageindex + "&keywords=" + keywords;	
	}
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo admin_url();?>masters/sites_del", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
	...
	{
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo http_url();?>admin/gifts/up_do/" + id, 
			type: 'GET', 
			dataType: 'html', 
			timeout: 15000, 
				error: function(){
	...
	{
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo http_url();?>admin/gifts/down_do/" + id, 
			type: 'GET', 
			dataType: 'html', 
			timeout: 15000, 
				error: function(){
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 站点管理 <span class="c-gray en">&gt;</span> 站点管理员信息 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
