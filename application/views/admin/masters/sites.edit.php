	...
	function article_save_submit()
	{
		...
		if(form_loads==1)
		{
			...
			else
			{
				...
				form_loads=2;	
				$.ajax({url:"<?php echo admin_url();?>masters/sites_updates/<?php echo $result['id'];?>", 
				type: 'POST', 
				data:{name:name,url:url,joins:joins,counts:counts,best:best,medium:medium,negative:negative}, 
				dataType: 'html', 
				timeout: 15000, 
					error: function(){
						layer.closeAll();
					...
					success:function(result){
						...
						if(result.indexOf("|")>0){
							...
							if(arr[0]==100){
								...
								setTimeout("location='<?php echo admin_url();?>masters/sites?keywords=<?php echo $keywords;?>&pageindex=<?php echo $pageindex;?>'",1500);	
