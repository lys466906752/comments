...
<script>
	...
	function reads()
	{
		$.ajax({url:"<?php echo admin_url();?>yins/index_ajax/" + pageindex + "?keywords=" + keywords, 
			type: 'GET', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	{
		location="<?php echo admin_url();?>yins/edit/" + id + ".html?pageindex=" + pageindex + "&keywords=" + keywords;	
	}
	...
	function ups(id)
	{
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo admin_url();?>yins/ups/" + id, 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
	...
	{
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo admin_url();?>yins/downs/" + id, 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo admin_url();?>yins/del", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 印章设置 <span class="c-gray en">&gt;</span> 印章列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c"> 
    <input type="text" class="input-text" style="width:250px" placeholder="印章名称" id="keys" name="keys" value="<?php echo $keywords;?>">&nbsp;&nbsp;
