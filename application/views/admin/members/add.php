	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<tbody>
			...
			<tr style="display:none;">
			  <td style="text-align:center;">添加方式：</td>
			  <td align="left" valign="middle"><input type="radio" name="addType" id="addType"  value="1" onClick="showShow();">
			    从现有会员中添加 
			      　　
			      <input type="radio" name="addType" id="addType" value="2" onClick="showShow();" checked>
			      设置一个新的会员</td>
		  </tr>
			<tr class="text-c" id="show_a" style="display:none;">
			  <td>会员手机号：</td>
			  <td align="left" valign="middle"><input type="text" class="input-text" value="" placeholder="请填写会员注册的手机号" id="mobile_a" name="mobile_a"></td>
		  </tr>
			<tr class="text-c" id="show_b1" style="display:none;">
			  <td>用户昵称：</td>
			  <td align="left" valign="middle"><input type="text" class="input-text" value="" placeholder="请填写用户昵称" id="nickname_b" name="nickname_b"></td>
		  </tr>
			<tr class="text-c" id="show_b2" style="display:none;">
			  <td>手机号：</td>
			  <td align="left" valign="middle"><input type="text" class="input-text" value="" placeholder="请填写会员注册的手机号" id="mobile_b" name="mobile_b"></td>
		  </tr>
			<tr class="text-c" id="show_b3" style="display:none;">
...
<script>
	...
	function article_save_submit()
	{
		...
		if(form_loads==1)
		{
			...
			var mobile_a=$("#mobile_a").val().replace(/(^\s*)|(\s*$)/g,"");
			...
			if(s_group_value==1 && mobile_a=="")
			{
				layer.msg('请填写会员手机号！');			
			}
			else if(s_group_value==1 && !mobile_check.test(mobile_a))
			{
				layer.msg('会员手机号填写有误');	
			}
			else if(s_group_value==2 && nickname_b=='')
			{
				layer.msg('请填写会员昵称！');		
			}
			...
			else if(s_group_value==2 && passwd_b=='')
			{
				layer.msg('请填写会员登录密码！');		
			}
			else if(s_group_value==2 && (passwd_b.length<6 || passwd_b.length>16))
			{
			...
			else
			{
				...
				form_loads=2;	
				$.ajax({url:"<?php echo admin_url();?>members/inserts", 
				type: 'POST', 
				data:{act:s_group_value,mobile_a:mobile_a,nickname_b:nickname_b,mobile_b:mobile_b,passwd_b:passwd_b}, 
				dataType: 'html', 
				timeout: 15000, 
					error: function(){
						layer.closeAll();
					...
					success:function(result){
						...
						if(result.indexOf("|")>0){
							...
							if(arr[0]==100){
								...
								setTimeout("location='<?php echo admin_url();?>members/index?keywords=<?php echo $keywords;?>&pageindex=<?php echo $pageindex;?>'",1500);
							}else if(arr[0]==200){
