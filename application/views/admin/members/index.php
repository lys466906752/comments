...
<script>
	...
	function reads()
	{
		$.ajax({url:"<?php echo admin_url();?>members/index_ajax/" + pageindex + "?keywords=" + keywords + '&state=' + state, 
			type: 'GET', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	{
		location="<?php echo admin_url();?>members/edit/" + id + "/member.html?pageindex=" + pageindex + "&keywords=" + keywords;	
	}
	...
	function del_it(id){
		layer.confirm('删除所选会员，他们发布的每一条评论和对应关联的评论都将被删除，请慎重！', {
			btn: ['确定','取消'] //按钮
		}, function(){
			del_rs(id);
		}, function(){
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 站点管理 <span class="c-gray en">&gt;</span> 普通会员信息 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c"> 
    <select id="fayan" name="fayan" class="input-text" style="width:150px;">
    	<option value="">--选择发言状态--</option>
        <option value="1">允许</option>
        <option value="2">禁止</option>
    </select>
    &nbsp;&nbsp;
    <input type="text" class="input-text" style="width:250px" placeholder="会员手机号/昵称" id="keys" name="keys" value="<?php echo $keywords;?>">&nbsp;&nbsp;
