...
<article class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<tbody>
			<tr class="text-c">
			  <td>用户昵称：</td>
			  <td align="left" valign="middle"><input type="text" class="input-text" value="<?php echo $result['nickname'];?>" placeholder="请填写用户昵称" id="nickname_b" name="nickname_b"></td>
		  </tr>
			<tr class="text-c">
			  <td>发言状态：</td>
              ...
              <option value="2"<?php if($result['state']==2){?> selected<?php }?>>禁止</option>
		      </select>
              </td>
		  </tr>
			<tr class="text-c">
			  <td>手机号：</td>
			  <td align="left" valign="middle"><input type="text" class="input-text" value="<?php echo $result['mobile'];?>" placeholder="请填写会员注册的手机号" id="mobile_b" name="mobile_b"></td>
		  </tr>
          <tr class="text-c">
			  <td width="164">账户余额：</td>
			  <td width="760" align="left" valign="middle">
              <input type="text" class="input-text" value="<?php echo $result['money'];?>" placeholder="请填写用户账户畅言豆余额，必须为整数，大于0" id="money_b" name="money_b">
              </td>
		  </tr>
          <tr>
              ...
              <img src="<?php echo base_url().$result['avatar'];?>" style="width:100px;height:100px" id="avatars">
              </td>
		  </tr>
          <tr>
			  <td width="164" style="text-align:center;">头像更新：</td>
			  <td width="760" align="left" valign="middle" style="text-align:left;">
              <form id="recsons" name="recsons" action="<?php echo admin_url();?>members/avatar_upload" method="post" enctype="multipart/form-data" class="definewidth m20" target="upload_target1">
...
<script>
	...
	function article_save_submit()
	{
		...
		if(form_loads==1)
		{
			...
			else if(s_group_value==2 && mobile_b!='' &&  !mobile_check.test(mobile_b))
			{
				layer.msg('会员手机号填写有误！');		
			}
			else if(money_b=='')
			...
			else
			{
				...
				form_loads=2;	
				$.ajax({url:"<?php echo admin_url();?>members/updates/<?php echo $result['id'];?>", 
				type: 'POST', 
				data:{money:money_b,nickname_b:nickname_b,mobile_b:mobile_b,passwd_b:passwd_b,avatar:tupian,state:state}, 
				dataType: 'html', 
				timeout: 15000, 
					error: function(){
						layer.closeAll();
					...
					success:function(result){
						...
						if(result.indexOf("|")>0){
							...
							if(arr[0]==100){
								...
								if(act.indexOf('master')>=0)
								{
									setTimeout("location='<?php echo admin_url();?>masters/index?keywords=<?php echo $keywords;?>&pageindex=<?php echo $pageindex;?>'",1500);	
								}
								else
								{
