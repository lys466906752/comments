...
<script>
	...
	function stopUpload(result){
		...
		if(result.indexOf("|")>0){
			...
			if(arr[0]==100){
				layer.msg('修改成功');
				setTimeout("location='<?php echo admin_url();?>faces/index?keywords=<?php echo $keywords;?>&pageindex=<?php echo $pageindex;?>'",1500);
			}else if(arr[0]==200){
				layer.msg('登录状态已失效');		
...
<form id="recsons" name="recsons" action="<?php echo admin_url();?>faces/index_update/<?php echo $result['id'];?>" method="post" enctype="multipart/form-data" class="definewidth m20" target="upload_target1">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<tbody>
			...
			<tr class="text-c" >
              ...
              <input type="text" class="input-text" value="<?php echo $result['name'];?>" placeholder="请填写表情名称，10个字以内" id="name" name="name">
              </td>
		  </tr>

          <tr >
              <td align="center" valign="middle" style=" text-align:center;">表情预览：</td>
              <td align="left" valign="middle"><img src="<?php echo base_url().$result['file'];?>" style="width:60px;"></td>
          </tr>
          <tr >
...
<script>
	...
	function article_save_submit()
	{
		...
		if(form_loads==1)
		{
			...
			var files=$("#files").val().replace(/(^\s*)|(\s*$)/g,"");
			...
			{
				layer.msg('表情名称必须少于10字');	
			}
			else if(files=="")
			{
				layer.msg('请选择上传的表情文件');	
			}
			else
			{
				layer.closeAll();
