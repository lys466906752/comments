<?php
	...
	{
		function get_csrfToken($obj,$session)
		{
			$time=time();
			$id=uniqid();
	...
	{
		function check_csrfToken($obj,$value,$session)
		{
			...
			if(count($arr==6))
			{
				$time=$arr[0];
				$id=$arr[2];
				if(trim(md5($time))==trim($arr[1]) && trim(md5($id))==trim($arr[3]) && trim(md5($time.$id))==trim($arr[4]) && time()-$time<7200 && trim($arr[5])==trim(csrfKeys))
				{
					return true;
				}
